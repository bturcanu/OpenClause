...
func (api *ConsoleAPI) handleSimulateTenantPolicy(w http.ResponseWriter, r *http.Request) {
	...
	if in.PolicyConfig != nil {
		...
		if existing, found, err := api.store.GetTenantPolicyConfig(r.Context(), tenantID); err == nil && found && existing != nil {
			cfg = *existing
		}
	}
	if err := validateTenantPolicyConfig(&cfg); err != nil {
	...
	if err != nil {
		...
		writeError(w, http.StatusBadGateway, "failed to read policy engine response")
		return
	}
	...
	if err := json.Unmarshal(respBody, &opaResp); err != nil {
		...
		writeError(w, http.StatusBadGateway, "invalid policy engine response")
		return
	}
